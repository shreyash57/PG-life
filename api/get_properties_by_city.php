...
$properties = $result_2 ? mysqli_fetch_all($result_2, MYSQLI_ASSOC) : [];
if (empty($properties)) {
    echo json_encode(["error" => "No properties found in this city."]);
    exit;
}
...
$interested_users_properties = $result_3 ? mysqli_fetch_all($result_3, MYSQLI_ASSOC) : [];

$new_properties = [];
foreach ($properties as $property) {
    $property_images = glob("../img/properties/" . $property['id'] . "/*.{jpg,jpeg,png,gif}", GLOB_BRACE);
    $property_image = !empty($property_images) ? 
